...
The main question is simple. Can a low-cost tabletop social robot, using a
general-purpose vision-language model, reliably support medication verification at
home? This paper makes the following contributions:
...
pill counting or color-based verification on a home robot. That is the contribution
of this work.
...
pill is their lisinopril. The robot confirms the match and logs the dose when the
user takes it. Later the user starts to take it again, and the robot warns that it
was already logged earlier that day.
\emph{Demo~3 (wrong count):} the user has a saved dose of a known size and presents a
tray with the wrong number of pills. The robot refuses to confirm and states the
specific problem, for example one extra pill. A fourth scripted interaction tests
the safety rule. When asked an interaction question, the robot declines and points
the user to a doctor or pharmacist.
...
\section{Conclusion}
A low-cost tabletop robot with a general-purpose VLM can support home medication
