...
from reachy_medication_app.config import config
from reachy_medication_app.prompts import get_session_voice, get_session_instructions
from reachy_medication_app.tools.core_tools import (
    ToolDependencies,
    get_tool_specs,
)
from reachy_medication_app.tools.background_tool_manager import (
    ToolCallRoutine,
    ToolNotification,
    BackgroundToolManager,
)
from reachy_medication_app.profiles._reachy_medication_app_locked_profile.set_reminder import (
    mark_reminders_delivered,
    pop_pending_reminders,
)
...
_OUTPUT_POLL_INTERVAL_S: Final[float] = 0.2
_IDLE_SIGNAL_DELAY_S: Final[float] = 300.0
...
def _looks_like_medication_label_question(question: str) -> bool:
...
def _exact_speech_instruction(text: str, followup_question: str = "") -> str:
    base = (
        "Say this naturally in one or two short sentences without adding new facts: "
        f"{json.dumps(text)}"
    )
    if followup_question:
        base += f" Then ask exactly this next question: {json.dumps(followup_question)}"
    return base
...
class OpenaiRealtimeHandler(AsyncStreamHandler):
    ...
    def __init__(self, deps: ToolDependencies, gradio_mode: bool = False, instance_path: Optional[str] = None):
        ...
        self._last_reminder_poll_monotonic: float = 0.0
    ...
    async def apply_personality(self, profile: str | None) -> str:
        ...
        try:
            ...
            from reachy_medication_app.config import config as _config
            from reachy_medication_app.config import set_custom_profile
    ...
    def _discard_pending_responses(self) -> None:
        """Drop any queued response.create payloads that belong to the old turn."""
        while True:
            try:
    ...
    def _has_pending_conversation_activity(self) -> bool:
        """Whether there is already tool or response work in flight.

        Used to suppress idle behaviors while the robot is still in the middle of
        a real task or has a queued spoken reply it has not delivered yet.
        """
        if not self._pending_responses.empty():
            return True
        if not self._response_done_event.is_set():
            return True
        return bool(self.tool_manager.get_running_tools())
    ...
    async def _handle_tool_result(self, bg_tool: ToolNotification) -> None:
        ...
        try:
            ...
            if bg_tool.tool_name == "camera" and "b64_im" in tool_result:
                ...
                if self.deps.camera_worker is not None:
                    ...
                    if np_img is not None:
                        # Camera frames are BGR; flip channels for Gradio display.
                        rgb_frame = np_img[..., ::-1].copy()
                    else:
                        rgb_frame = None
            ...
            if not bg_tool.is_idle_tool_call:
                self.last_activity_time = asyncio.get_event_loop().time()
                await self._cancel_active_response_for_barge_in()
                if hasattr(self, "_clear_queue") and callable(self._clear_queue):
                    self._clear_queue()
                followup_instructions = "Use the tool result just returned and answer concisely in speech."
                if isinstance(tool_result, dict):
                    spoken_summary = str(tool_result.get("spoken_summary", "")).strip()
                    followup_question = str(tool_result.get("followup_question", "")).strip()
                    if spoken_summary:
                        followup_instructions = _exact_speech_instruction(spoken_summary, followup_question)
                if bg_tool.tool_name == "scan_bottle" and followup_instructions.startswith("Use the tool result"):
                    followup_instructions = (
                        "Use the scan_bottle result just returned. If label_clear is true, read back the drug "
                        "name, strength, and instructions in short speech and ask the user to confirm. If "
                        "label_clear is false, say you could not read the bottle clearly and ask them to hold "
                        "it closer, steadier, or in better light."
                    )
                elif bg_tool.tool_name == "count_pills" and followup_instructions.startswith("Use the tool result"):
                    followup_instructions = (
                        "Use the count_pills result just returned. Tell the user the total visible pill count "
                        "first, then the color breakdown if available. If the count is unclear, say so plainly "
                        "and ask them to spread the pills out in the tray and try again."
                    )
                elif bg_tool.tool_name == "check_medications" and followup_instructions.startswith("Use the tool result"):
                    followup_instructions = (
                        "Use the check_medications result just returned. Give a brief tray summary focused on "
                        "visible pill count, colors, and any containers. If the view was unclear, say that and "
                        "ask the user to reposition the tray items."
                    )
                elif bg_tool.tool_name == "verify_dose" and followup_instructions.startswith("Use the tool result"):
                    followup_instructions = (
                        "Use the verify_dose result just returned. If safe_to_proceed is true, confirm briefly "
                        "that the pills look right. If safe_to_proceed is false, explain the specific mismatch "
                        "or uncertainty and tell the user to double-check before taking anything."
                    )
                elif bg_tool.tool_name == "camera":
                    camera_question = ""
                    if isinstance(tool_result, dict):
                        camera_question = str(tool_result.get("question", "")).strip()
                    if _looks_like_medication_label_question(camera_question):
                        followup_instructions = (
                            "Use the camera result to read the medication bottle label. Read back only the "
                            "details you are actually confident about, say clearly what is still unclear, and "
                            "ask the user to confirm or correct the medication details in one short follow-up "
                            "question."
                        )
                    elif followup_instructions.startswith("Use the tool result"):
                        followup_instructions = (
                            "Use the camera result and answer the user's visual question briefly. If the view is "
                            "insufficient, say so plainly and suggest looking again from another angle."
                        )
    ...
    async def _cancel_active_response_for_barge_in(self) -> None:
        """Best-effort cancel of the active response when the user cuts in."""
        if not self.connection:
            return
        ...
        try:
            ...
            logger.debug("response.cancel during barge-in ignored: %s", e)

        # Wake any waiting response sender so the next turn can proceed cleanly.
        self._response_done_event.set()

    async def _interrupt_for_user_speech(self) -> None:
        """Immediately yield the robot back to the user when they start talking."""
        self.last_activity_time = asyncio.get_event_loop().time()
        ...
        try:
            ...
            logger.debug("clear_move_queue during speech interrupt ignored: %s", e)

        for bg_tool in list(self.tool_manager.get_running_tools()):
            try:
                await self.tool_manager.cancel_tool(bg_tool.tool_id, log=False)
            except Exception as e:
                logger.debug("cancel_tool during speech interrupt ignored for %s: %s", bg_tool.tool_id, e)
            ...
            try:
                ...
                async for event in self.connection:
                    ...
                    if event.type == "input_audio_buffer.speech_started":
                        await self._interrupt_for_user_speech()
                    ...
                    if event.type == "response.function_call_arguments.done":
                        ...
                        if self.is_idle_tool_call:
                            ...
                            self.last_activity_time = asyncio.get_event_loop().time()
                            running_instructions = "Briefly tell the user what you are doing while the tool runs."
                            if tool_name == "scan_bottle":
                                running_instructions = (
                                    "Say one short sentence like: 'Okay, let me look at the bottle.'"
                                )
                            elif tool_name in ("count_pills", "check_medications", "verify_dose"):
                                running_instructions = (
                                    "Say one short sentence like: 'Okay, let me check the tray.'"
                                )
                                ...
                                try:
                                    ...
                                    question = ""
                                if _looks_like_medication_label_question(question):
                                    running_instructions = (
                                        "Say one short sentence like: 'Okay, let me read the bottle.'"
                                    )
                                else:
                                    running_instructions = (
                                        "Say one short sentence like: 'Okay, let me take a look.'"
                                    )
                            elif tool_name == "add_medication":
                                running_instructions = (
                                    "Say one short sentence like: 'Okay, I'll save that.'"
                                )
                            elif tool_name == "set_reminder":
                                running_instructions = (
                                    "Say one short sentence like: 'Okay, I'll set that reminder.'"
                                )
                            await self._safe_response_create(
                                response={
                    ...
                    if event.type == "error":
                        ...
                        if code == "conversation_already_has_active_response":
                            ...
                            logger.debug("response.cancel ignored because no active response was running")
                        else:
    ...
    async def emit(self) -> Tuple[int, NDArray[np.int16]] | AdditionalOutputs | None:
        ...
        # Proactively surface fired reminders instead of waiting for the user to ask.
        now = asyncio.get_event_loop().time()
        if self.connection and (now - self._last_reminder_poll_monotonic) >= 1.0:
            self._last_reminder_poll_monotonic = now
            try:
                pending_reminders = await pop_pending_reminders()
            except Exception as e:
                logger.debug("Reminder poll skipped: %s", e)
                pending_reminders = []
            ...
            if pending_reminders:
                ...
                if reminder_text:
                    ...
                    try:
                        ...
                        logger.debug("clear_move_queue before reminder ignored: %s", e)
                    if hasattr(self, "_clear_queue") and callable(self._clear_queue):
                        self._clear_queue()
                    await self._cancel_active_response_for_barge_in()
                    await self.connection.conversation.item.create(
                            ...
                            "content": [
                                {
                                    "type": "input_text",
                                    "text": (
                                        f"[Reminder just fired at {time.strftime('%H:%M:%S')}] "
                                        f"Tell the user this now: {reminder_text}"
                                    ),
                                },
                            ],
                        },
                    ...
                    await self._safe_response_create(
                        response={
                            "instructions": _exact_speech_instruction(reminder_text),
                            "tool_choice": "none",
                        },
                    )
                    await mark_reminders_delivered(pending_reminders)

        # Handle idle
        idle_duration = asyncio.get_event_loop().time() - self.last_activity_time
        if (
            idle_duration > _IDLE_SIGNAL_DELAY_S
            and self.deps.movement_manager.is_idle()
            and not self._has_pending_conversation_activity()
        ):
            try:
        ...
        try:
            return await asyncio.wait_for(self.output_queue.get(), timeout=_OUTPUT_POLL_INTERVAL_S)
        except asyncio.TimeoutError:
            return None
