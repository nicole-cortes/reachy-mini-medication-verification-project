...
class Camera(Tool):
    ...
    name = "camera"
    description = (
        "Take a picture with the camera and ask a question about it. Use this for "
        "normal stock-style visual behavior like 'what do you see', and also for "
        "reading a medication bottle label when the user shows one."
    )
    parameters_schema = {
        "type": "object",
        "properties": {
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> Dict[str, Any]:
        ...
        if deps.vision_manager is not None:
            ...
            return (
                {"image_description": vision_result, "question": image_query}
